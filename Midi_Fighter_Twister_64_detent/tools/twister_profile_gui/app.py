...
class TwisterGui(Tk):
    ...
    def _build_ui(self) -> None:
        ...
        toolbar_wrap = ttk.Frame(profile_frame)
        toolbar_wrap.pack(fill=X, padx=8, pady=(8, 2))

        toolbar_r1 = ttk.Frame(toolbar_wrap)
        toolbar_r1.pack(fill=X, pady=(0, 3))
        ttk.Button(toolbar_r1, text="Load JSON", command=self.load_json).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Save JSON", command=self.save_json).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r1, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r1, text="Profile Metadata", command=self.open_profile_metadata_editor).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Compare Profile", command=self.compare_profile_file).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Merge Profile", command=self.merge_profile_file).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r1, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r1, text="Import Show Pack", command=self.import_everything_bundle).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Export Show Pack", command=self.export_everything_bundle).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Import Bank Snippet", command=self.import_bank_snippet).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Export Bank Snippet", command=self.export_bank_snippet).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r1, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r1, text="Restore Last Snapshot", command=self.restore_last_snapshot).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="Export Diagnostics", command=self.export_diagnostics_report).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r1, text="GitHub Patcher", command=self.open_github_patcher).pack(side=LEFT, padx=4)

        toolbar_r2 = ttk.Frame(toolbar_wrap)
        toolbar_r2.pack(fill=X, pady=(0, 3))
        ttk.Button(toolbar_r2, text="Undo", command=self.undo).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r2, text="Redo", command=self.redo).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r2, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r2, text="Pull Global", command=self.pull_global).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r2, text="Push Global", command=self.push_global).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r2, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r2, text="Pull Bank", command=self.pull_bank).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r2, text="Push Bank", command=self.push_bank).pack(side=LEFT, padx=4)
        ttk.Separator(toolbar_r2, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(toolbar_r2, text="Pull All Banks", command=self.pull_all_banks).pack(side=LEFT, padx=4)
        ttk.Button(toolbar_r2, text="Push All Banks", command=self.push_all_banks).pack(side=LEFT, padx=4)

        selector_wrap = ttk.Frame(profile_frame)
        selector_wrap.pack(fill=X, padx=8, pady=(6, 0))

        selector_r1 = ttk.Frame(selector_wrap)
        selector_r1.pack(fill=X, pady=(0, 3))
        ttk.Label(selector_r1, text="Bank").pack(side=LEFT)
        ttk.Spinbox(selector_r1, from_=1, to=4, width=4, textvariable=self.bank_var, command=self._on_selection_changed).pack(side=LEFT, padx=4)
        ttk.Label(selector_r1, text="Encoder").pack(side=LEFT, padx=(14, 0))
        ttk.Spinbox(selector_r1, from_=1, to=16, width=4, textvariable=self.encoder_var, command=self._on_selection_changed).pack(side=LEFT, padx=4)
        ttk.Label(selector_r1, text="Scope").pack(side=LEFT, padx=(14, 0))
        ttk.Combobox(selector_r1, textvariable=self.apply_scope_var, values=list(SCOPE_FIELDS.keys()), width=14, state="readonly").pack(side=LEFT, padx=4)
        ttk.Button(selector_r1, text="Load Active", command=self._load_encoder_fields_from_model).pack(side=LEFT, padx=8)
        ttk.Button(selector_r1, text="Apply To Selected", command=self._apply_encoder_fields_to_model).pack(side=LEFT, padx=4)
        ttk.Button(selector_r1, text="Apply Favorites", command=self.apply_favorites_to_selected).pack(side=LEFT, padx=4)

        selector_r2 = ttk.Frame(selector_wrap)
        selector_r2.pack(fill=X, pady=(0, 3))
        ttk.Button(selector_r2, text="Preview Diff", command=self.preview_diff_selected).pack(side=LEFT, padx=4)
        ttk.Button(selector_r2, text="Heatmap Selected", command=self.preview_heatmap_selected).pack(side=LEFT, padx=4)
        ttk.Button(selector_r2, text="Heatmap All", command=self.preview_heatmap_all).pack(side=LEFT, padx=4)
        ttk.Button(selector_r2, text="Clear Heatmap", command=self.clear_heatmap).pack(side=LEFT, padx=4)
        ttk.Separator(selector_r2, orient="vertical").pack(side=LEFT, fill=Y, padx=8)
        ttk.Button(selector_r2, text="Send Selected", command=self.push_selected_encoder).pack(side=LEFT, padx=4)

        safety_wrap = ttk.Frame(profile_frame)
        safety_wrap.pack(fill=X, padx=8, pady=(6, 2))

        safety_r1 = ttk.Frame(safety_wrap)
        safety_r1.pack(fill=X, pady=(0, 3))
        ttk.Checkbutton(safety_r1, text="Dry Run", variable=self.dry_run_var).pack(side=LEFT)
        ttk.Label(safety_r1, text="Confirm if sending >=").pack(side=LEFT, padx=(10, 2))
        ttk.Spinbox(safety_r1, from_=1, to=64, width=4, textvariable=self.confirm_threshold_var).pack(side=LEFT)
        ttk.Label(safety_r1, text="encoders").pack(side=LEFT, padx=(4, 0))
        ttk.Separator(safety_r1, orient="vertical").pack(side=LEFT, fill=Y, padx=10)
        ttk.Checkbutton(safety_r1, text="Performance Mode", variable=self.performance_mode_var).pack(side=LEFT)
        ttk.Label(safety_r1, text="Delay(ms)").pack(side=LEFT, padx=(8, 2))
        ttk.Spinbox(safety_r1, from_=0, to=30, width=4, textvariable=self.performance_delay_ms_var).pack(side=LEFT)
        ttk.Label(safety_r1, text="Retry").pack(side=LEFT, padx=(8, 2))
        ttk.Spinbox(safety_r1, from_=0, to=3, width=3, textvariable=self.performance_retry_var).pack(side=LEFT)

        safety_r2 = ttk.Frame(safety_wrap)
        safety_r2.pack(fill=X, pady=(0, 3))
        ttk.Button(safety_r2, text="Compat Check", command=self.show_firmware_compatibility_report).pack(side=LEFT, padx=4)
        ttk.Button(safety_r2, text="Recovery Mode", command=self.guided_recovery_mode).pack(side=LEFT, padx=4)
        ttk.Separator(safety_r2, orient="vertical").pack(side=LEFT, fill=Y, padx=10)
        ttk.Button(safety_r2, text="Start Sandbox", command=self.start_sandbox).pack(side=LEFT, padx=4)
        ttk.Button(safety_r2, text="Commit Sandbox", command=self.commit_sandbox).pack(side=LEFT, padx=4)
        ttk.Button(safety_r2, text="Discard Sandbox", command=self.discard_sandbox).pack(side=LEFT, padx=4)
        ttk.Label(safety_r2, textvariable=self.sandbox_status_var).pack(side=LEFT, padx=(8, 0))
